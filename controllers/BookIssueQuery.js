...
const viewBookissueRecords = async (req,res) =>{
    try {
        const viewQuery = "SELECT book_issue_id , book_name , student_name ,students.stud_id,issue_date ,due_date ,return_date,late_fine,is_submmited FROM book_issue_details INNER JOIN students ON book_issue_details.stud_id = students.stud_id INNER JOIN books ON book_issue_details.book_id = books.book_id"
...
const updateBookissueRecord = async (req,res) => {
  ...
  const totalIssueBookRecord = async (req,res)=>{
    try {
        const countQuery="SELECT COUNT(*) AS count FROM book_issue_details"
        const [row,field] = await db.execute(countQuery)
        res.status(200).json({message:row})
    } catch (error) {      
        res.status(500).json({message:'Internal server error'}) 
    }
...
module.exports={insertBookissueRecord,viewBookissueRecords,updateBookissueRecord,totalIssueBookRecord}