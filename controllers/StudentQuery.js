...
const viewStudentSingleRecords = async (req,res) =>{
    try {
        const {id} = req.params
        const viewQuery = "select student_name, class as student_class ,mobile,password  from students where stud_id=?"
        const [row,field]=await db.execute(viewQuery,[id])
        res.status(200).json({message:row})
    } catch (error) {
        res.status(500).json({message:'Internal server error'})
    }
...
const totalStudentRecord = async (req,res)=>{
    try {
...
module.exports={insertStudentRecord,viewStudentRecords,viewStudentSingleRecords,updateStudentRecord,deleteStudentRecord,totalStudentRecord}